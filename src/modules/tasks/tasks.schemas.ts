...
export const CreateTaskSchema = z.object({
  title: z.string().min(3).max(120), description: z.string().max(2000).optional(), type: z.enum(TASK_CATEGORIES),
  totalBudget: z.number().positive(), rewardPerSlot: z.number().positive(), totalSlots: z.number().int().min(1).max(10000),
...
// Status is accepted here only for backwards-compatible pause/resume requests.
// The route rejects mixed content+status mutations, and the service never accepts protected states.
export const UpdateTaskSchema = z.object({
  title: z.string().min(3).max(120).optional(), description: z.string().max(2000).optional(),
  link: z.string().url().optional(), proofInstructions: z.string().max(1000).optional(),
  status: z.enum(["active", "paused"]).optional(),
});

export const UpdateTaskStatusSchema = z.object({ status: z.enum(["active", "paused"]) });

export const ListTasksQuery = z.object({
  cursor: z.string().optional(), limit: z.coerce.number().int().min(1).max(50).default(20), type: z.string().optional(), status: z.string().default("active"),
});
