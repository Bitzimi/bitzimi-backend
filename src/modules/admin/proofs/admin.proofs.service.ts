...
export async function adminDecideProof(reviewId: string, reviewerId: string, decision: "approved" | "rejected", note?: string) {
  ...
  await db.$transaction(async tx => {
    const reviewGuard = await tx.adminProofReview.updateMany({
      where: { id: reviewId, decision: null },
    ...
    const proofGuard = await tx.taskProof.updateMany({
      where: { id: review.proofId, status: "review", rewardPaid: false },
      data: { status: decision === "approved" ? "admin_approved" : "admin_rejected", rewardPaid: decision === "approved", processedAt: now },
    });
    ...
    const rewardAmount = Number(proof.rewardAmount ?? 0);
  ...
  setImmediate(() => createNotification({
    ...
    message: approved ? "Your task proof has been reviewed and approved. Reward has been credited to your task wallet." : `Your task proof was reviewed and rejected${note ? `: ${note}` : "."}`,
