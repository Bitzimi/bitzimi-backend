...
async function debitAcrossWallets(
  ...
  for (const walletType of WITHDRAWAL_WALLETS) {
    ...
    // Prevents a concurrent withdrawal from reducing the balance between the initial
    // findMany read and this update (PostgreSQL READ COMMITTED double-spend race).
    const result = await tx.wallet.updateMany({
      where: { userId, walletType, balance: { gte: debitAmount } },
