...
export async function writeLedgerEntry(tx: any, entry: {
  ...
  if (entry.referenceType === "game_bet" && entry.referenceId) {
    const bet = await tx.gameBet.findUnique({ where: { id: entry.referenceId }, select: { amount: true, round: { select: { gameType: true, lobbyId: true } } } });
    if (bet) metadata = { gameType: bet.round.gameType, stake: bet.amount, ...(bet.round.lobbyId ? { lobby: bet.round.lobbyId } : {}), ...metadata };
  }

  // PVP settlement rows use referenceType=pvp_match. Enrich the win/loss ledger
  // records from the authoritative match row so the transaction history can show
  // the actual game and stake/room without guessing from the amount.
