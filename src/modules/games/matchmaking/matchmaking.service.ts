...
export async function joinQueue(userId: string, gameType: MatchGameType, stake: number) {
  ...
  try {
    ...
    const match = await createReservedMatchForPlayers(userId, opponent.userId, gameType, stake);
...
export async function createMatchForPlayers(player1Id: string, player2Id: string, gameType: MatchGameType, stake: number) {
  ...
  if (gameType === "dice_clash") { try { await resolveDiceClash(match.id, player1Id, player2Id, stake, fee, totalPool, serverSeed, feeRate); } catch (err) { console.error(`[Matchmaking] Immediate settlement deferred for ${match.id}:`, err); } }
  else if (gameType === "pvp_coinflip") await resolveCoinFlip(match.id, player1Id, player2Id, stake, fee, totalPool, serverSeed, feeRate);
...
async function recoverUnresolvedImmediateMatches(){const matches=await db.pvpMatch.findMany({where:{status:"active",gameType:{in:["dice_clash"]}},take:100});for(const m of matches){const feeRate=await getGameFeeRate(m.gameType as MatchGameType);const totalPool=m.stake*2;const fee=totalPool*feeRate;if(!m.serverSeed)continue;try{await resolveDiceClash(m.id,m.player1Id,m.player2Id,m.stake,fee,totalPool,m.serverSeed,feeRate)}catch{}}}
