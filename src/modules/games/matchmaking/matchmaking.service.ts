...
export async function createMatchForPlayers(player1Id: string, player2Id: string, gameType: MatchGameType, stake: number) {
  ...
  const match = await db.$transaction(async tx => {
    for (const pid of [player1Id, player2Id]) {
      ...
      await writeLedgerEntry(tx, { userId: pid, type: "game_bet", fromWallet: "game", amount: stake, description: `${gameType} match entry`, referenceType: "pvp_match", metadata: { gameType, stake } });
    }
  ...
  try {
    ...
    // The funded active match is deliberately retained. The recovery worker retries settlement.
    console.error(`[Matchmaking] Immediate settlement deferred for ${match.id}:`, err);
  }
...
async function resolveDiceClash(matchId: string, p1Id: string, p2Id: string, stake: number, fee: number, totalPool: number, serverSeed: string, feeRate: number) {
  ...
  const winnerId = p1Roll > p2Roll ? p1Id : p2Roll > p1Roll ? p2Id : (p1Id < p2Id ? p1Id : p2Id);
  const loserId = winnerId === p1Id ? p2Id : p1Id;
  const payout = totalPool - fee;
  await db.$transaction(async tx => {
    const guard = await tx.pvpMatch.updateMany({ where: { id: matchId, status: "active" }, data: { status: "settled", winnerId, resultData: JSON.stringify({ p1Roll, p2Roll, winnerId }), settledAt: new Date(), serverSeed, clientSeed, nonce: 1 } });
...
async function resolveCoinFlip(matchId: string, p1Id: string, p2Id: string, stake: number, fee: number, totalPool: number, serverSeed: string, feeRate: number) {
  ...
  const payout = totalPool - fee;
  await db.$transaction(async tx => {
    const guard = await tx.pvpMatch.updateMany({ where: { id: matchId, status: "active" }, data: { status: "settled", winnerId, resultData: JSON.stringify({ p1Side: "heads", p2Side: "tails", coinFlip, winnerId }), settledAt: new Date(), serverSeed, clientSeed, nonce: 1 } });
    if (!guard.count) return;
    await creditWallet(tx, winnerId, "game", payout);
...
export async function getMatch(userId: string, matchId: string) {
  const match = await db.pvpMatch.findFirst({ where: { id: matchId, OR: [{ player1Id: userId }, { player2Id: userId }] }, include: { player1: { include: { profile: { select: { username: true } } } }, player2: { include: { profile: { select: { username: true } } } } } });
  ...
  const fee = totalPool * await getGameFeeRate(match.gameType as MatchGameType);
  return { matchId: match.id, gameType: match.gameType, stake: match.stake, totalPool, platformFee: fee, status: match.status, isPlayer1, opponent: { username: opponent.profile?.username ?? "Player", userId: opponent.id }, result: match.resultData ? JSON.parse(match.resultData) : null, winnerId: match.winnerId, youWon: match.winnerId === userId, payout: match.winnerId === userId ? totalPool - fee : 0, createdAt: match.createdAt.toISOString(), settledAt: match.settledAt?.toISOString() ?? null, signalSentAt: match.signalSentAt?.toISOString() ?? null, yourReady: isPlayer1 ? match.player1Ready : match.player2Ready, opponentReady: isPlayer1 ? match.player2Ready : match.player1Ready };
}

export async function signalReady(userId: string, matchId: string) {
  return db.$transaction(async tx => {
    const match = await tx.pvpMatch.findFirst({ where: { id: matchId, status: "active", OR: [{ player1Id: userId }, { player2Id: userId }] } });
    if (!match) throw Object.assign(new Error("Match not found"), { statusCode: 404, code: "NOT_FOUND" });
    await tx.$queryRaw`SELECT id FROM pvp_matches WHERE id = ${matchId} FOR UPDATE`;
    const isPlayer1 = match.player1Id === userId;
    const data: any = isPlayer1 ? { player1Ready: true } : { player2Ready: true };
    const updated = await tx.pvpMatch.update({ where: { id: matchId }, data });
    if (updated.player1Ready && updated.player2Ready && !updated.signalSentAt) {
      ...
      const final = await tx.pvpMatch.update({ where: { id: matchId }, data: { signalSentAt } });
      return { signalSentAt: final.signalSentAt!.toISOString(), delayMs };
    }
    return { waiting: true };
  });
}
...
export async function submitTap(userId: string, matchId: string, tapMs: number) {
  if (!Number.isFinite(tapMs) || tapMs < -60_000 || tapMs > 60_000) throw Object.assign(new Error("Invalid reaction time"), { statusCode: 400, code: "INVALID_TAP" });
  const result = await db.$transaction(async tx => {
    const match = await tx.pvpMatch.findFirst({ where: { id: matchId, status: "active", OR: [{ player1Id: userId }, { player2Id: userId }] } });
    if (!match || !match.signalSentAt) throw Object.assign(new Error("Signal not yet sent"), { statusCode: 400, code: "SIGNAL_NOT_SENT" });
    await tx.$queryRaw`SELECT id FROM pvp_matches WHERE id = ${matchId} FOR UPDATE`;
    const isPlayer1 = match.player1Id === userId;
    const updateData: any = isPlayer1 ? { player1TapMs: tapMs } : { player2TapMs: tapMs };
    const updated = await tx.pvpMatch.update({ where: { id: matchId }, data: updateData });
    return updated;
  });
  if (result.player1TapMs !== null && result.player2TapMs !== null) await resolveReactionTap(matchId, result.player1Id, result.player2Id, result.stake, result.player1TapMs, result.player2TapMs);
  return { submitted: true };
}

async function resolveReactionTap(matchId: string, p1Id: string, p2Id: string, stake: number, p1Tap: number, p2Tap: number) {
  const p1Forfeited = p1Tap < 0, p2Forfeited = p2Tap < 0;
  ...
  const winnerId = p1Forfeited ? p2Id : p2Forfeited ? p1Id : p1Tap === p2Tap ? (p1Id < p2Id ? p1Id : p2Id) : p1Tap < p2Tap ? p1Id : p2Id;
  const loserId = winnerId === p1Id ? p2Id : p1Id;
  await db.$transaction(async tx => {
    const guard = await tx.pvpMatch.updateMany({ where: { id: matchId, status: "active" }, data: { status: "settled", winnerId, resultData: JSON.stringify({ p1TapMs: p1Tap, p2TapMs: p2Tap, winnerId, p1Forfeited, p2Forfeited }), settledAt: new Date() } });
    if (!guard.count) return;
    await creditWallet(tx, winnerId, "game", payout);
...
async function processReactionTapTimeouts() {
  const now = new Date();
  const matches = await db.pvpMatch.findMany({ where: { gameType: "reaction_tap", status: "active", signalSentAt: { not: null, lt: new Date(now.getTime() - REACTION_TAP_TIMEOUT_MS) } }, take: 100 });
  for (const m of matches) {
    const p1 = m.player1TapMs, p2 = m.player2TapMs;
    if (p1 !== null && p2 !== null) await resolveReactionTap(m.id, m.player1Id, m.player2Id, m.stake, p1, p2);
    else if (p1 !== null) await resolveReactionTap(m.id, m.player1Id, m.player2Id, m.stake, p1, -1);
    else if (p2 !== null) await resolveReactionTap(m.id, m.player1Id, m.player2Id, m.stake, -1, p2);
    else await resolveReactionTap(m.id, m.player1Id, m.player2Id, m.stake, -1, -1);
  }
}

async function recoverUnresolvedImmediateMatches() {
  const matches = await db.pvpMatch.findMany({ where: { status: "active", gameType: { in: ["dice_clash", "pvp_coinflip"] } }, take: 100 });
  for (const m of matches) {
    const feeRate = await getGameFeeRate(m.gameType as MatchGameType);
    const totalPool = m.stake * 2;
    const fee = totalPool * feeRate;
    if (!m.serverSeed) continue;
    try {
      if (m.gameType === "dice_clash") await resolveDiceClash(m.id, m.player1Id, m.player2Id, m.stake, fee, totalPool, m.serverSeed, feeRate);
      else await resolveCoinFlip(m.id, m.player1Id, m.player2Id, m.stake, fee, totalPool, m.serverSeed, feeRate);
    } catch {}
  }
}
