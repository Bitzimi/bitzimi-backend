...
async function resolveDiceClash(matchId: string, p1Id: string, p2Id: string, stake: number, fee: number, totalPool: number, serverSeed: string, feeRate: number) {
  ...
  // Section D: commission on BOTH players' fees
  const userFee = stake * feeRate;
  setImmediate(() => triggerGameFeeCommission({ userId: winnerId, userFee, isMultiGame: false, eventRefId: matchId }));
  setImmediate(() => triggerGameFeeCommission({ userId: loserId,  userFee, isMultiGame: false, eventRefId: matchId }));
}
