...
export async function settleSingleWinner(result: SingleWinnerResult): Promise<void> {
  ...
  await db.$transaction(async (tx) => {
    ...
    // Record winner stat
    await recordGameResult({ tx, userId: winnerId, gameType, wagered: totalPool / (loserIds.length + 1), won: true, payout: winnerPayout });

    // Record loser stats
    const loserStake = totalPool / (loserIds.length + 1);
    for (const loserId of loserIds) {
      await recordGameResult({ tx, userId: loserId, gameType, wagered: loserStake, won: false, payout: 0 });
    }
  });
