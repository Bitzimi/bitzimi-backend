...
function parseTeam(value: string): "red" | "blue" { return JSON.parse(value).team as "red" | "blue"; }

async function getCanonicalRound(dailyRoundNumber: number) {
  return db.gameRound.findFirst({ where: { gameType: "color_game", lobbyId: "A", dailyRoundNumber }, orderBy: { startedAt: "asc" } });
}
...
export async function getGlobalColorResult(dailyRoundNumber: number) {
  ...
  if (existing?.result === "red" || existing?.result === "blue") return { result: existing.result as "red" | "blue", clientSeed: canonical.clientSeed ?? generateClientSeed("color-global", String(dailyRoundNumber)), serverSeed: canonical.serverSeed };
...
async function settleRecoveredRound(round: any, bets: any[], global: { result: "red" | "blue"; clientSeed: string; serverSeed: string }) {
  ...
  const roundNumber = round.dailyRoundNumber ?? round.roundNumber;

  if (isVoid) {
    await db.$transaction(async tx => {
      for (const bet of bets) {
        await creditWallet(tx, bet.userId, "game", bet.amount);
        await writeLedgerEntry(tx, { userId: bet.userId, type: "transfer", toWallet: "game", amount: bet.amount, description: "Color game void — full refund", referenceId: bet.id, referenceType: "game_bet", metadata: { gameType: "color_game", roundId: round.id, roundNumber, voided: true } });
        ...
        await tx.notification.create({ data: { userId: bet.userId, type: "game_void", title: "⚠️ Color Prediction Round Voided", message: `Round ${roundNumber} was voided because only one side had bets. Your ${bet.amount.toFixed(2)} stake was fully refunded.`, metadata: JSON.stringify({ game: "color_game", roundId: round.id, roundNumber, payout: bet.amount, voided: true }) } });
      }
    });
  ...
  await db.$transaction(async tx => {
    for (const bet of bets) {
      ...
      if (won) {
        ...
        await writeLedgerEntry(tx, { userId: bet.userId, type: "game_win", toWallet: "game", amount: payout, description: `Color game win — team ${global.result}`, referenceId: bet.id, referenceType: "game_bet", metadata: { team, winner: global.result, roundId: round.id, roundNumber, stake: bet.amount, payout } });
        await tx.notification.create({ data: { userId: bet.userId, type: "game_win", title: "🎉 Color Prediction Victory!", message: `You won ${payout.toFixed(2)} on ${global.result.toUpperCase()} in round ${roundNumber}.`, metadata: JSON.stringify({ game: "color_game", roundId: round.id, roundNumber, team, winner: global.result, stake: bet.amount, payout }) } });
      } else {
        await writeLedgerEntry(tx, { userId: bet.userId, type: "game_loss", fromWallet: "game", amount: bet.amount, description: `Color game loss — team ${team}`, referenceId: bet.id, referenceType: "game_bet", metadata: { team, winner: global.result, roundId: round.id, roundNumber, stake: bet.amount, payout: 0 } });
        await tx.notification.create({ data: { userId: bet.userId, type: "game_loss", title: "Color Prediction Result", message: `You lost ${bet.amount.toFixed(2)} on ${team.toUpperCase()} in round ${roundNumber}.`, metadata: JSON.stringify({ game: "color_game", roundId: round.id, roundNumber, team, winner: global.result, stake: bet.amount, payout: 0 }) } });
      }
...
export async function recoverStaleColorRounds() {
  ...
  recoveryRunning = true; lastRecoveryAt = now;
  try {
    const stale = await db.gameRound.findMany({ where: { gameType: "color_game", status: { in: ["waiting", "spinning", "result"] }, dailyRoundNumber: { lt: await currentDailyRoundNumber() } }, orderBy: { startedAt: "asc" }, take: 50 });
    for (const round of stale) {
      ...
      const dailyRoundNumber = round.dailyRoundNumber ?? round.roundNumber;
      const global = await getGlobalColorResult(dailyRoundNumber);
      if (!global) continue;
