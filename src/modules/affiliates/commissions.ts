...
/**
 * Create a game-fee CommissionJob record inside an existing Prisma transaction.
 *
 * Call this INSIDE a settlement db.$transaction so the commission job is
 * guaranteed to exist if the settlement commits. This eliminates the window
 * where a post-settlement DB failure could silently drop the commission.
 *
 * Every game service uses this instead of the post-settlement setImmediate approach.
 */
export async function createGameFeeJobInTx(
  tx: any,
  opts: { userId: string; userFee: number; isMultiGame: boolean; eventRefId: string }
): Promise<void> {
  if (opts.userFee <= 0) return;
 ...
 * LEGACY: prefer createGameFeeJobInTx() called inside the settlement transaction.
 * This function is kept as a fallback for callers that cannot be restructured.
 * If all 3 DB write retries fail the commission is permanently lost.
 */
export async function triggerGameFeeCommission(opts: {
  ...
  // Game settlement has already completed, so we cannot roll it back —
  // but we must persist the commission job or the upline earns nothing.
  const MAX_ENQUEUE_ATTEMPTS = 3;
  for (let attempt = 1; attempt <= MAX_ENQUEUE_ATTEMPTS; attempt++) {
    try {
      await db.commissionJob.create({ data: jobData });
      return;
    } catch (err) {
      if (attempt === MAX_ENQUEUE_ATTEMPTS) {
        console.error(`[Commission] Failed to enqueue game fee job after ${MAX_ENQUEUE_ATTEMPTS} attempts (userId=${opts.userId}, eventRefId=${opts.eventRefId}):`, err);
        return;
      }
