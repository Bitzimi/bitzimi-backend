 ...
 *
 * Tests critical platform flows against a live PostgreSQL test database.
 * Run with: DATABASE_URL="postgresql://user:pass@localhost:5432/bitzimi_test" npx tsx --test tests/e2e.test.ts
 *
...
async function buildTestApp(): Promise<FastifyInstance> {
  ...
  const { adminKycRoutes }      = await import("../src/modules/admin/kyc/admin.kyc.routes");
  ...
  app.register(adminKycRoutes,     { prefix: "/api/v1/admin/kyc" });
...
before(async () => {
  // Set test-environment config so deposit/withdrawal flows can execute without
  // real external services. These values are fake and safe — only used in the
  // isolated test DB that is destroyed after the suite runs.
  process.env.CRYPTO_DEPOSIT_ADDRESS = "TBitzimiTestWalletAddress1234";
  process.env.BANK_DEPOSITS_ENABLED  = "true";
...
after(async () => {
  ...
  await db.$disconnect();
  // PostgreSQL test DB cleanup is handled externally (drop/recreate schema between runs).
  // No local file to delete.
  // Force exit — Prisma's query engine subprocess keeps Node alive otherwise
  process.exit(0);
});

// ── 1. Registration ───────────────────────────────────────────────────────────
test("POST /auth/register — creates user, 7 wallets, KYC stub", async () => {
  ...
  // Email verification is required before login. Simulate backend email-click
  // by directly marking the account verified in the test DB.
  const { db } = await import("../src/db");
  await db.user.update({
    where: { email: "e2e_user@bitzimi.com" },
...
test("GET /wallets — all 7 wallets provisioned at zero", async () => {
  const r = await get(app, "/api/v1/wallets", accessToken);
  assert.equal(r.status, 200);
  const b = r.body.data.balances;
  assert.equal(Object.keys(b).length, 7, "Should have exactly 7 wallets");
...
// ── 12. VIP subscription ──────────────────────────────────────────────────────
// KYC is checked before balance. The test user has no approved KYC, so
// the endpoint correctly returns 403 KYC_REQUIRED rather than 400 INSUFFICIENT_BALANCE.
test("POST /vip/subscribe — requires KYC approval (403 without approved KYC)", async () => {
  const r = await post(app, "/api/v1/vip/subscribe", {}, accessToken);
  assert.equal(r.status, 403, "Should fail — KYC not approved");
  assert.equal(r.body.error.code, "KYC_REQUIRED");
});
...
// ── 19. KYC authorization — normal user cannot approve ───────────────────────
test("POST /admin/kyc/:id/approve — normal user gets 403 (admin.kyc.approve required)", async () => {
  // Use a known submission ID that does not exist — the auth/permission check fires before DB lookup
  const fakeId = "non-existent-kyc-submission-id";
  const r = await post(app, `/api/v1/admin/kyc/${fakeId}/approve`, {}, accessToken);
  assert.equal(r.status, 403, `Normal user should be forbidden; got ${r.status}: ${JSON.stringify(r.body)}`);
  assert.ok(
    r.body.error?.code === "FORBIDDEN" || r.body.error?.code === "INSUFFICIENT_PERMISSION",
    `Expected FORBIDDEN or INSUFFICIENT_PERMISSION, got: ${r.body.error?.code}`
  );
});
